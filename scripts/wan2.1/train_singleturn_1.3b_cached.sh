...
export OUTPUT_DIR=${OUTPUT_DIR:-/mnt/cpfs/jiachengliu/dataset/CORNE/ckpt/1.3B_nocrossattn}
...
SINGLETURN_ROLLOUT_REVERSED=${SINGLETURN_ROLLOUT_REVERSED:-0}
SINGLETURN_ROLLOUT_SHUFFLED=${SINGLETURN_ROLLOUT_SHUFFLED:-0}
SINGLETURN_ROLLOUT_LINEAR_INTERP=${SINGLETURN_ROLLOUT_LINEAR_INTERP:-0}
SINGLETURN_ROLLOUT_SLERP_INTERP=${SINGLETURN_ROLLOUT_SLERP_INTERP:-0}
...
if [[ "${SINGLETURN_ROLLOUT_REVERSED}" == "1" ]]; then
  cmd+=(
    --singleturn_rollout_reversed
  )
fi

if [[ "${SINGLETURN_ROLLOUT_SHUFFLED}" == "1" ]]; then
  cmd+=(
    --singleturn_rollout_shuffled
  )
fi

if [[ "${SINGLETURN_ROLLOUT_LINEAR_INTERP}" == "1" ]]; then
  cmd+=(
    --singleturn_rollout_linear_interp
  )
fi

if [[ "${SINGLETURN_ROLLOUT_SLERP_INTERP}" == "1" ]]; then
  cmd+=(
